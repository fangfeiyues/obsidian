...
```text
    ...
    所以要防止 Domain 扩散到 App，罪魁祸首是 app 随意对 domain 的各种概念进行调用，如
...
- **查询类**
	...
	往往说查询类都是一个动作，可能就会把查询类操作定义成领域服务，如app走domian查询entity数据，或 app走domian查repository等，但其实不对，其一是查询链路有点多余，其二领域服务只能是一个动作而查询显然不是。所以查询应该是要归属到实体或聚合上，即在app层直接走实体类单个查询即可，同样多实体直接聚合
