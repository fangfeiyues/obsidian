...
-  **1、物流架构**
	...
	**与中台的关系**
		...
		1. 有赞寄件承接中台物流，负责物流商运营、计费与结算，包括商家拉黑、营销计费与佣金结算
		2. 上门取件作为中台逆向交易一环，上有商家同意退货、下有商家同意退款等状态流转
	
	***重难点***
	
		1. 有赞寄件增值业务边界，负责一切跟费用相关问题（什么时候到谁那扣多少费）。
			如连锁场景下总部为分店发货，那决定扣费方的到底是业务即告诉我们是总部，
			还是结算侧自己通过分店去找能扣费的
		2. 有赞寄件 限流 + 重试 + 离线对账 ？？
		3. 上门取件A/B测试决策最佳物流渠道 ？？
		4. 上门取件物流开放
...
-  **2、履约架构**
	...
	![[Pasted image 20250305165241.png|600]]
...
-  **3、物流增长**
	...
	**商家画像**
	...
	takerate 抽佣率，在金融行业收入/GMV，衡量一个业务盈利能力的标准
	
		1. 下一步物流增长目标是什么？- 调研商家画像，探索进一步的渗透
		2. 系统化低 + 自己发货：如夫妻店等重体验，可营销活动、送优惠
		3. 系统化高 + 自己发货：中小企业，电子面单/ERP -> 寄件的一客一价
		4. 系统化低 + 供应商发货：有成熟稳定服务，暂不考虑
		5. 系统化高 + 供应商发货：有成熟稳定服务，可与ERP合作
...
-  **4、履约增长**
	...
	**业务指标**
		...
		提高渗透率 + 降低赔付率（提高近90天退款率的均价） = 提升毛利率
		1. 渗透率 15%、
		2. 赔付率 77% =（退换货订单 * 赔付单价）/（订单数 * 定价均价），其中定价主要与近90天退货退款有关
		3. 毛利率 25%
...
-  **1、分销领域模型**
	...
	**问题**
		...
		1. 领域划分
			1. 各域之间、上下文之间的松耦合，可以独立替换或扩展。如客户关系、分佣计算规则等可以有三方
			2. 关于开放边界的考虑，要产品层整体考虑。如分佣结果、规则等也要归属
		2. 落地方式
			1. 自上而下
			2. 自下而上
			3. 场景落地
	...
	**好处**
		...
		1. 抽象稳定：不会DAO满天飞如订单域去更新关系数据统计数据
		2. 平台话：领域层通过SPI调用下游，而不是应用层直接去
