...
Redis 是一个事件驱动处理器，需处理以下两类事件
	...
	1、文件事件：Redis服务器通过`套接字`与`客户端`连接，文件事件就是服务器对 套接字 操作的抽象
...
-  **优势**
	...
	2.  fork 子进程，不需主进程进行任何磁盘IO操作
	3.  恢复速度快

#### RDB 快照

	快照是 Redis 把数据由内存同步到硬盘的过程，时间会较长

-  **数据一致性**

	核心思路是 `Copy-on-Write 边写边复制`
	1、主进程会 fork 一个新的快照进程专门来做这个事情，这样保证了Redis服务不会停止对客户端包括写请求在内的任何响应；
	2、同时发生的数据变化会以副本的方式存放在另一个新的内存区域，待快照操作结束后才会同步到原来的内存区域
...
-  **服务 -> 奔溃**
	...
	也就是说，在快照操作过程中不能影响上一次的备份数据。Redis服务会在磁盘上创建一个临时文件进行数据操作，待操作成功后才会用这个临时文件替换掉上一次的备份。

-  **自动快照时间**

	快照时间过于频繁的话，虽然不会阻塞主线程，但也会带来两方面的开销
...
-  **触发方式**
	...
	1.  `always` 写入 aof_buf 并 同步到 AOF文件
...
![[image-Redis-1 存储-20240616234757462.png]]
...
-  **内存淘汰算法**
	...
	Redis共支持八种淘汰策略： `noeviction、volatile-random、volatile-ttl、volatile-lru、volatile-lfu、allkeys-lru、allkeys-random 和 allkeys-lfu`
	分三类
	- 不淘汰
	    - noeviction （v4.0后默认的）
	- 对设置了过期时间的数据中进行淘汰
