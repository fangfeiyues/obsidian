...
Sentinel 设置局部零头的规则是先到先得：最先向目标Sentinel发送 is-master-down-by-addr 设置要求的，而之后接受到所有设置都会被目标Sentinel拒绝。得到半数以上的Sentinel将成为领头。否则将再次选举，就是一个 Raft选举算法 [[Distributed-1  分布式基论 CAP]]
